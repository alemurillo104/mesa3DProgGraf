...
{
    class RightArm : Parte
    {
        float valor = 1;
        HashList<Parte> extremos;

        public RightArm(Vector3 centro, float anchoX, float altoY, float profZ)
        {
            ...
            extremos.Add("rsuperior", as1);
            extremos.Add("rinferior", as2);
        ...
        public override TexturedVertex[] GetVerts2()
        {
            ...
            TexturedVertex[] x = extremos.Get("rsuperior").GetVerts2();
            TexturedVertex[] y = extremos.Get("rinferior").GetVerts2();
        ...
        public override void setScale(float s, bool plus)
        {
            foreach (DictionaryEntry parte in extremos)
            {
                if (plus)
                {
                    extremos.Get(parte.Key).MoverX(0.005f);
                    extremos.Get(parte.Key).setScale(s, true);
                    //valor+=1.05f;
                }
                else
                {
                    extremos.Get(parte.Key).MoverX(-0.005f );
                    extremos.Get(parte.Key).setScale(s, false);
                    //valor -= 1.05f;
                }
            }
        }
